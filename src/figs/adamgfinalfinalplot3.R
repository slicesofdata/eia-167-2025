...
#  GET DATA 
fetch_us_price_sector <- function(sector_id) {
  eia_data(
...
#  PERCENT CHANGE
# 2001 is the baseline for each sector
us_pct <- us_prices %>%
  group_by(sector) %>%
  mutate(
    base_price = price_cents_kWh[year == YEAR_START],
    pct_change = 100 * (price_cents_kWh / base_price - 1)
  ) %>%
  ungroup()
...
p_us_pct <- ggplot(us_pct, aes(x = year, y = pct_change)) +
  ...
  geom_point(aes(color = sector, shape = sector), size = 2) +
  ...
  scale_y_continuous(
    labels = percent_format(scale = 1, accuracy = 1)  # 10%, 20%, ...
  ) +
  labs(
    title = sprintf("Percent Change in U.S. Electricity Prices by Sector (%d–%d)", 
